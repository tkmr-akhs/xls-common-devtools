...
Public Sub Test_SetAlignment_OmittedArguments_StoresNoChangeValuesInDictionary(ByVal Assert As UnitTestAssert)
    ' Arrange
    Dim test_dbl As WorksheetServiceTestDouble: Set test_dbl = New WorksheetServiceTestDouble
    Dim sheet_srv As IWorksheetService: Set sheet_srv = test_dbl
    Dim range_bounds As WorksheetRangeBounds
    ...
    stored_value = test_dbl.SetAlignment_Results.Items()(0)
    ' Array(HorizontalAlignment, VerticalAlignment, Orientation, IndentLevel)
    Assert.EqualsNumeric G_ALIGNMENT_NO_CHANGE, stored_value(0)
    Assert.EqualsNumeric G_ALIGNMENT_NO_CHANGE, stored_value(1)
    Assert.EqualsNumeric G_ALIGNMENT_NO_CHANGE, stored_value(2)
    Assert.EqualsNumeric G_ALIGNMENT_NO_CHANGE, stored_value(3)
End Sub
